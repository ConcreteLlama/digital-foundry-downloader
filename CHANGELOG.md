...
A new Backfill tool under Tools, for applying things to content you already have rather than only to whatever you download next. Pick a set of videos and generate subtitles for them, analyse them, or find Digital Foundry's written article for each - with a button that selects everything still missing the thing you picked, so catching up on a library does not mean clicking through it item by item.

Everything runs as a single background job you can watch and cancel from the Activity page, rather than hundreds of separate ones. It re-checks each item as it reaches it, so a long run will not re-transcribe something that gained subtitles while it was queued, and analysis runs tell you what they will cost before you start them.
