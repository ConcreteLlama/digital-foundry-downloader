...
Your downloads can now be read as well as watched. Point it at an Anthropic API key, or run a model on your own machine for nothing, and it will write a summary and a separate verdict for each video, pull the hard numbers out into a table you can actually compare - per-platform resolutions and frame rates, or a PC review's settings - and suggest tags. It also finds Digital Foundry's own written article where one exists and reads that alongside the transcript, which matters more than it sounds: an article is written rather than transcribed, so the product names and figures in it are right where speech-to-text mangles exactly that kind of jargon.

A new Analysis section reads across everything you have analysed rather than one video at a time, and a new Backfill tool under Tools applies any of this to content you already have rather than only to whatever you download next. Digital Foundry's articles are also picked up on their own from now on, so something you download today gains its written companion when that appears next week.

Downloads play in the app itself now, with their subtitles and a list of what is in the video beside them, so jumping to the one section you care about no longer means finding the file on disk. And Plex or Jellyfin can be told the moment a file changes, so a finished download turns up in your library straight away instead of waiting for their next scan.
...
- AI analysis of your content
  ...
  - The hard numbers pulled out where the content supports it - per-platform resolutions, frame rates and modes for a platform comparison, the settings table for a PC review, and a per-topic breakdown for a Q+A rather than one summary flattening several unrelated discussions
  - Developer interviews get that topic-by-topic breakdown as well. These are some of the most detailed things Digital Foundry publish and they previously produced a summary and nothing else, so an engine deep dive came out as prose with no way to jump to the part about lighting or streaming
  - Every finding now says where its quote came from. One taken from the video gets a timestamp you can click to jump to the moment; one taken from Digital Foundry's written article is marked as such, because an article has no moment to jump to. Previously the second kind showed nothing at all, which made a perfectly good citation look like the analysis had made something up
  ...
  - A button to check your API key works before anything depends on it. It only counts tokens rather than analysing anything, so it costs nothing and cannot leave a half-finished analysis attached to a video - and the same button checks a local model server is reachable
- Or run it on your own machine instead, with no API key and nothing to pay
  ...
  - Two models to choose from, with the trade-off spelled out where you pick one. The smaller is the default and the recommendation for almost everyone: it runs on a 12GB graphics card, is around three times faster, and finds more of what it is looking for - on one settings table it listed ten settings where the larger model listed six, and all four extras were real. The larger one quotes the video more accurately, so more findings end up with a timestamp you can click, and it writes fuller summaries - but wherever the answer is a list it returns fewer entries, and it needs around 22GB of memory to hold, realistically a 32GB machine. Richer writing, less complete tables
  - Locally, the transcript is now annotated with positions before the video is picked apart for findings. That helps the model quote the video accurately rather than approximately, which is what decides whether a finding gets a clickable timestamp at all. It reads more of the transcript to do it, so on a very long video it is skipped rather than risk not fitting
  - Worth knowing what you give up: locally the summaries are shorter and the write-up thinner than Haiku manages. It works out the content type and the game just as reliably. Fewer of its findings carry a timestamp you can click, though that is mostly not a failing on its part - where Digital Foundry published a written article it often cites that rather than the video, which is a correct citation that simply has no moment to jump to
...
- Tell Plex or Jellyfin the moment something changes
  - A finished download turns up in your library straight away, instead of whenever your media server next gets round to scanning on its own schedule
  - Not only new downloads. Metadata written into a file, subtitles generated for it later, and files shifted about by the Reorganize tool all count as changes, so a video that gains subtitles an hour after you downloaded it is picked up too
  - Plex is told exactly which folder changed and rescans only that, rather than combing through your entire library every time a video lands
  - Moving a file tells the server about both the old folder and the new one, so the entry disappears from where it used to be instead of lingering as something that is no longer there
  - Changes are batched. A download, the metadata written into it and the subtitles placed beside it are one refresh a few seconds after things go quiet, not three scans of a folder while it is still being written to - you can change how long it waits
  - Path mapping for the usual case where the two do not agree on what a folder is called, which is almost always when either side runs in a container. Once you have tested the connection it offers the folders your server itself reported, so you pick from a list rather than typing a path that fails silently if you get it wrong, and it shows you what your download folder will actually be sent as
  - A Test connection button for each, which says what it found - for Plex, the libraries it can see - or what went wrong in terms you can act on, distinguishing a wrong token from a server that is switched off from an address that cannot be resolved
  - Watching a download in the app's own player marks it watched on your media server, and records where you got to, so something you have already seen does not sit there looking unwatched. One direction only for now - watching in Plex does not change anything here
  - Jellyfin needs you to sign in for this, rather than the API key above, because what was watched belongs to a person rather than to the server. Your password is exchanged for a token and not stored
  - Worth knowing for Jellyfin: it has no built-in way to rescan a single folder, so it gets a whole-library refresh. If you install the Targeted Scans plugin this uses it and the scan becomes near-instant whatever the size of your library
...
- A new Analysis section, reading across everything you have analysed
  - Platform comparisons: every analysed platform comparison side by side, with Digital Foundry's own recommendation quoted against each
...
- Analyse your back catalogue overnight, on a schedule
  - Set hours when the app may work through everything it has not analysed yet - by default midnight until 5am, every day - and it gets on with it while you are not using the machine. Found under Settings then Scheduled backfill
  - This exists for local analysis in particular, where a single video can take tens of minutes and you would rather that happened at 3am than while you are trying to use the computer. Scheduling Claude runs is offered too, where the point is deciding when you spend rather than what the machine is doing - so a spending cap per night and a rough cost appear for those and stay out of the way for local ones
  - Say when it starts and when it stops, and it tells you how long that is - including "ends next day" for something like 10pm until 2am, so a window crossing midnight needs no arithmetic and no guessing
  - The window decides when analyses are allowed to *start*, not when they are cut off. One already running at 5am finishes rather than being abandoned part-way, because throwing away twenty minutes of work at a clock boundary helps nobody - the panel says this where the times are, so the first time it runs past the hour it does not look broken
  - It queues one video at a time and waits for that to finish before adding the next. That is what keeps the overrun to a single video rather than however many were queued at once, and it means a fast machine gets through more per night and a slow one fewer without you having to tell it anything about either
  - Anything you start yourself jumps ahead of it. Scheduled work sits at the lowest priority, so asking for an analysis at 2am does not mean waiting behind the overnight queue, and a download finishing at 6am is not held up by it either
  - Choose what it picks up: only videos with subtitles, only ones with a Digital Foundry article, or both. The article condition genuinely matters for quality rather than being a preference, so it is on by default
  - Waiting for an article can have a time limit - analyse anyway if none has appeared within a fortnight, say - or you can switch that off and require an article however long it takes, which is a reasonable thing to want and was worth making possible
  - It says what will happen before it happens: when the next window opens, how many videos are eligible, and - when none are - why, so "nothing eligible" reads as finished rather than broken
  - Recent nights are listed with what each managed: how many were analysed, how many failed, and whether it ran out of time or ran out of work
  - The server's own clock and time zone are shown under the times, since that is what the schedule actually runs on and a server in another zone is exactly the thing that catches people out
  - If no analysis engine is set up the switch is unavailable rather than hidden, and says what is missing and where to fix it. A schedule with nothing behind it would do nothing, silently, forever - and silently doing nothing is exactly what a working schedule looks like
  - For anyone who wants something more specific than the day and time controls offer, there is a raw cron expression behind Advanced, shown back to you in plain English, and it refuses to accept an expression it cannot read rather than quietly meaning "never"
- It remembers what you have watched
  - Watched and part-watched markers on every row and card, with a progress bar along the thumbnail so you can see at a glance what you started and never finished
  - Filter the list by not started, part-watched or watched - the quickest way to find what is left in a back catalogue you have been working through
  - Where you got to now survives closing the tab. Before this it was remembered only until the page was reloaded, so anything longer than a few minutes had to be found again by hand
  - This is kept by the app itself and works whether or not you run a media server, because "have I seen this" is worth answering on its own
  - Plex and Jellyfin can keep it in step, both ways: what you watch in the app marks it there, and what you watch there is pulled back in. Watched is sticky, so a server that has never heard of a file cannot un-watch something you definitely saw
  - Pulled in on a schedule you set, and there is a Sync now button for when you have just set it up and want to see it work rather than wait for the timer
  - It reports what each server recognised, so a path mapping that matches nothing says so instead of looking like there was simply nothing to do
  - Jellyfin needs only its API key now, and you pick which account from a list. It used to ask for your Jellyfin username and password purely to learn which user you were, which it never actually needed
### Enhancements
...
- The metadata backfill can now tell which files actually need it
  ...
  - Files downloaded before this existed read as "Never written" rather than up to date. That is deliberate: nothing recorded what went into them, and claiming they are current would quietly skip the ones most likely to need it
...
- The content list shows what has been analysed, and what has an article
  - Two small marks on each row, so you can see at a glance which of your videos have been analysed and which have a written Digital Foundry piece attached, without opening each one
  - The analysis mark distinguishes a thorough reading from a rough one: an analysis based on a transcript or an article is shown solid, while one drawn only from a title and description is dimmed, because that is a much weaker basis and worth knowing before you trust a tag
  - Costs nothing to show - it reads what the app already has in memory, so the list is no slower to open
...
- Media server connections are now three separate switches
  - Connecting to a server, telling it to rescan when files change, and keeping watched state in step are separate choices rather than one. A machine that never serves media has no use for rescans but every use for reading watched state, and previously the only way to get the second was to accept the first
  - Path mapping has moved up to sit with the server's address and key, since it is part of setting the connection up rather than of either job - and both jobs break in the same way when it is wrong
- The settings section list now scrolls on its own, so it stays put while you scroll a long settings page rather than disappearing off the top with it
- Your browser no longer offers to save API keys as passwords. Keys and tokens are still hidden on screen, but they are no longer the kind of field a password manager tries to remember - the app's own login is untouched
### Bug Fixes
- Reorganize Files now brings subtitle files along with the video. They were left behind in the old folder, which quietly broke them - a subtitle file only works while it sits next to its video - and left directories that could never be tidied away
- Marking something watched on Plex or Jellyfin now happens once, rather than every ten seconds for the whole last tenth of a video
- The log now says which server was told to rescan and why, so a library that is not updating can be diagnosed from the log rather than by guessing
- The dismiss button on a notification works. It could be swiped away but the X did nothing, because pressing it started a drag on the notification behind it and the button never saw the click
...
- The Activity page makes more sense while a backfill runs
  ...
  - A finished job no longer describes itself as still working. It kept showing the last thing it was doing mid-way through, so a completed job read as though it were still going
  - The activity count in the top bar now includes background jobs like backfills. It only counted downloads, so starting a backfill of three hundred items showed nothing at all at the moment the most was happening
- The status column lines up again
  - A video with two downloaded files has a longer line of detail than one with a single file, and that was pushing its whole status block sideways - so "Downloaded" and "Available" sat at different places down the page and the column read as ragged rather than as a column
...
- Subtitle generation no longer looks stuck for the first part of every run
  - Two real stages happen before transcribing starts - pulling the audio out of the video, then loading the speech model - and neither said anything at all. The job sat with an empty bar and stale text, which reads as hung rather than as working
  - Pulling the audio out now reports its own progress, and the moment transcription begins it says it is loading the model, so what is on screen always describes what is actually happening instead of whatever the previous stage left there
  - This covers subtitles generated automatically after a download just as much as ones you ask for by hand
### Security
...
- Analysis now reads the video's chapter markers, which it never did before
  - Chapters are written by Digital Foundry rather than transcribed, so the names in them are spelt correctly where speech-to-text garbles exactly that - and for a Direct they are effectively the list of what it covered, which is the hardest thing to recover from a transcript
  - For anything downloaded they are read back out of the file itself, which costs nothing and reaches nothing: they are already embedded in every download. A single item you analyse by hand may also check YouTube if its file has none; bulk runs deliberately never do, since that would attach a request to every item of a several-hundred item run
  - They are also used on the cheapest runs, where there is no transcript at all - a chapter list is the only account of what is in the video at that point
- Choose which sources an analysis reads, per run
  - Both the Analyse button on a piece of content and a bulk run now ask which sources to use at the point you press go, rather than leaving a control sitting on the page to be walked past. The starting selection is whatever your AI analysis settings say, and changing it applies to that run only rather than writing back to your settings
  - The sources are all listed, including the ones you cannot turn off. Title and description are always read, so they are shown as fixed rather than left out - listing only the two switchable ones made it look as though the run read nothing else
  - The estimated cost re-prices as you change them, because the difference is not small: on a real selection, dropping the subtitles took the estimate from $0.08 to $0.01
  - This is mostly about cost. The transcript is nearly all of what a run is charged for, so turning it off makes a run dramatically cheaper - at the price of the summary, the verdict, the structured breakdown and the jump-to timestamps, which the picker says outright rather than leaving you to discover afterwards
  - Turning the article off also stops the app looking one up, so a bulk run over hundreds of items does not make hundreds of requests to Digital Foundry for something it has been told not to read
  - Defaults for all of this live under Settings, AI analysis
- The Backfill tool now explains what each source adds to an analysis, and roughly what each costs, on the AI analysis tab where you are about to spend money on one. The step that matters is not gradual: with neither subtitles nor an article there is nothing to read, so those runs produce tags and a classification but no summary, verdict or breakdown however good the title is - and they cost a fraction as much, which is the reason to want them. The tiers that read subtitles scale with how long the video is, so the figures shown are for an ordinary video rather than a ceiling
- Coming back to a tab left in the background no longer replays every progress bar. A hidden tab keeps receiving updates but stops drawing, so on returning each bar animated from where it was last drawn up to where it had actually got to - which, with several transcriptions running, looked alarmingly like the work being done all over again. Bars now jump straight to where they are, which is the truth: that progress happened while you were elsewhere
- Finished subtitle jobs say "Generated en subs with whisper" rather than "Fetched". Whisper transcribes on your own machine and fetches nothing - the wording dated from when every subtitle service was a remote one
...
- Analysis sections are easier to scan
  ...
  - Cards carry a coloured edge and a tinted heading, and the platform comparison table gives each game a matching edge and a heavier rule between rows - its rows are tall enough that a hairline was easy to lose when reading across the columns
  - A little more colour generally: the headline of each item, the engine and hardware-class labels, and the performance figures now pick up the theme's accents rather than everything being the same grey
...
- Two new sections under Analysis: PC Settings and Hardware
  ...
  - Performance costs are shown per game and never averaged across them. A figure comes from one scene on one test machine at one resolution, so a number pooled from several reviews would describe nothing. Where a cost was only described in words the page leaves it blank and says so - in practice only about one setting in seven gets a number, and a column of blanks should not look like a fault
  - Hardware collects reviews of graphics cards, CPUs, handhelds and displays, which previously appeared in no section at all. Games used to benchmark a card are listed as tests rather than as coverage, so a review never looks like it was about them
  - Reviews are listed rather than merged per product: Digital Foundry revisit hardware as drivers and prices change, so two reviews of the same card are two verdicts at two moments rather than one entry that has to pick between them
- Analysis now records which game a video is about, for far more of your library
  - The Games section only ever listed platform comparisons and PC reviews, because those were the only two kinds of analysis that recorded a game at all. Everything else - a hands-on, a Switch 2 port, a patch revisit, a retrospective - was analysed and then filed nowhere, even though each is about exactly one game
  - Looking across a full library, about a fifth of content was being filed where over half of it is about a specific game. That gap is now closed, and the biggest single category it was missing - one game examined on one platform, the ports and patches and 'have they fixed it yet' pieces - gets its own treatment including what actually changed since the last version
  - Directs, Q+As and round-ups now record the games they covered as well. They have no single subject, so they never appear as coverage of one game - but a Direct that spent five minutes on a remaster announcement is often the only record you have of what was said about it, and it now turns up under that game rather than being invisible
  ...
  - Analysis of hardware reviews, interviews, tech explainers and news shows is now classified as such rather than landing in "Other", so the label above an analysis tells you what it actually is
...
- Notifications no longer have to interrupt you
  ...
  - Snooze for 15 minutes, an hour or four - which is usually what you want when you have just queued a few hundred jobs and know exactly what is about to happen
- The top bar is a row of icons rather than a line of prose. "Idle" took up most of the space on a phone to say nothing was happening; activity is now an icon with a count, and the detail it used to spell out - what is downloading, what is processing, the transfer rate - is in its tooltip and one tap from the page itself
- Tags produced by an analysis now reach the file. Analysis runs before metadata is written during a download, but the step was embedding the tags the content had when the download started, so anything the analysis had just produced was saved to the library and left out of the file
- Fixed metadata being written to a file and then immediately overwritten with the version that has none. The step that files a finished download checks whether metadata was embedded first, and had been reading the result of the wrong step ever since analysis was added to the pipeline - so with analysis switched off it concluded injection had failed every time, and replaced the tagged file with the raw download
...
- The Activity page is now grouped by the kind of work, not by stage
  ...
  - Each section says how many are queued and shows a marker when something in it is running, and long queues show the first few with the rest folded away rather than hundreds of near-identical rows
  - Buttons to show everything, or only what is running, queued or held - so 'what is this machine doing right now' is one press rather than a scroll
  - Queued items can be reordered in more places than before, not just downloads. Where reordering would not mean anything it is not offered rather than being offered and ignored
  - Work that is running and cannot be interrupted is pinned in place. Dragging something above it would have pushed it out of the running slot, and since that kind of job cannot actually be paused it would have carried on regardless while the app started another one on top of it
