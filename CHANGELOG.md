...
- Choose which sources an analysis reads, per run
  - Both the Analyse button on a piece of content and a bulk run now ask which sources to use before spending anything - the subtitles, Digital Foundry's article, or neither. The starting selection is whatever your AI analysis settings say, and changing it applies to that run only rather than writing back to your settings
