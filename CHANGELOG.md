...
- Resume or start again, as a choice rather than a guess
  - A part-watched video shows how far through you are and puts two buttons on the thumbnail - resume from where you stopped, or play from the start. Anything you have not started still plays on a press, as it always did
  - The time offered is the one you actually stopped at, including when you have only just closed the player
### Bug Fixes
- A video you are watching again no longer looks finished in the lists
  - Marking something watched is deliberately sticky, so starting it again left the row showing a tick and a full progress bar while you were only part way back through it. The row now shows both - that you have seen it, and where you actually are
  - A video left sitting on the credits still just reads as watched, which is what it is
- The player's controls get out of the way in fullscreen
  ...
  - They stay put while paused, while a menu is open, and while you are tabbing through them, since those are all moments where having them vanish would be worse than leaving them
...
### Enhancements
- The backfill lists open on what still needs doing
  - Not analysed and Upgradeable are shown together by default, and the state buttons add up rather than being one choice - complete items are most of the list and cannot be acted on without turning re-runs on
  - The controls are grouped as well: what narrows the list, which states to show, and what to do about it. They were one long row that broke wherever the width ran out, which on a phone put a filter next to Run
...
- Smaller fixes
  ...
  - On screens above 1200px the left edge of switches and checkboxes was cut off, most visibly in Settings - the panel they sit in had started trimming anything that reached past its edge
...
Downloads play in the app itself, with their subtitles and the video's chapters beside them. It also keeps track of what you have watched, and can keep that in step with Plex or Jellyfin - which are now told the moment a file changes, so a download appears in your library straight away.

A new Analysis section reads across everything you have analysed at once, and a Backfill tool applies any of this to content you already have - overnight on a schedule if you would rather it ran while you are not using the machine.
...
- AI analysis of your content
  ...
  - The hard numbers pulled into a table where the content supports it - per-platform resolutions, frame rates and modes for a platform comparison, the settings table for a PC review, and a per-topic breakdown for a Q+A or a developer interview rather than one summary flattening several unrelated discussions
  - Findings carry the moment they were said, so a result or a settings row is one click from that point in the video. The time is found rather than guessed - the analysis quotes the video word for word and the app locates that quote in the subtitles, so a jump either lands where the thing was said or is not offered at all
  ...
  - You choose the model, and see what a run will cost before starting it - from a real token count, not a generic figure. Haiku is the default and the recommendation: in testing it extracted settings tables and per-platform numbers correctly at around a tenth of the cost of the larger models
  - Run it by hand per video, or automatically after each download. Automatic waits for subtitles first, so it has a transcript to work from rather than quietly producing a thinner result
- Or run it on your own machine instead, with no API key and nothing to pay
  ...
  - Two models, with the trade-off spelled out where you pick one. The smaller is the default and right for almost everyone: it runs on a 12GB graphics card, is around three times faster, and finds more wherever the answer is a list - on one settings table it listed ten where the larger model listed six, and all four extras were real
  - It will not fight subtitle generation for the machine - if Whisper is transcribing, an analysis waits its turn rather than both crawling. The model is released once it has been idle a while rather than sitting on memory
  - Worth knowing what you give up: locally the summaries are shorter and fewer findings carry a clickable timestamp. It works out the content type and the game just as reliably
- Suggested tags, including for content you have not downloaded
  - Tagging is the one part that does not need a transcript - a title and description are enough to infer something useful, so it works across your whole library rather than only what you have downloaded
  - Every tag records what it was inferred from, shown next to it, because one drawn from a title alone is a weaker claim and you may be filtering on it
- Digital Foundry's written articles, found and used
  - Where DF published a written companion for a video it is found, linked on the content, and read alongside the transcript when analysing
  - This measurably improves the result. An article is written rather than transcribed, so its product names and figures are correct where speech-to-text reliably garbles them, and for PC reviews it often contains the settings table outright
  ...
  - New articles are noticed on their own, and the archive is worked backwards in the background, so older videos gain their companion without you going and asking. An Articles section lists everything found
- A new Analysis section, reading across everything you have analysed
  - Platform comparisons side by side, with Digital Foundry's own recommendation quoted against each, and filterable to the platforms you care about
  - What was covered per game, grouping the several videos DF often publish about one release
  - Nothing is averaged, scored or ranked. There is no best-platform column because the figure needed for one - what a mode actually ran at, rather than what it aims for - is stated in only about a tenth of cases
- Backfill: apply subtitles, analysis or article matching to content you already have
  ...
  - Select all that need it picks out everything still missing whatever you chose, so catching up on a back catalogue is one click rather than a scroll through the library
  - It runs as one job on the Activity page that you can pause or cancel, and work already finished is kept - stopping a long article search does not throw away the matches it found
  - Costs are shown before you commit: an estimated total for analysis, worked out from real pricing, and how many requests article matching will make to Digital Foundry
- Watch a downloaded video in the app
  - Press the still at the top of a downloaded video and it opens in a player, with its subtitles and a list of what is in the video beside it - so jumping to the one section you care about no longer means finding the file on disk
  - The chapters written into the file and the moments the analysis found are shown together in time order. Whichever row you are inside is highlighted and keeps up as the video plays, and clicking any row jumps there
  - Only the part being watched is sent, so a multi-gigabyte video starts almost immediately and seeking anywhere in it is instant, rather than transferring the whole file first
  - Theater mode gives the video nearly the whole window. Full screen, seeking, speed and captions are the browser's own controls, so they behave as they do on any other site
  - When a file will not play you are told this machine has no decoder for it, and where the file is - rather than being shown a black rectangle. The check asks your browser what it can actually decode instead of assuming from the format
- Tell Plex or Jellyfin the moment something changes
  - A finished download turns up in your library straight away instead of whenever your server next scans. Metadata written into a file, subtitles generated later, and files shifted by the Reorganize tool all count
  - Plex is told exactly which folder changed and rescans only that. Moving a file tells the server about both the old folder and the new one, so the entry does not linger where it used to be
  - Changes are batched - a download, its metadata and its subtitles are one refresh after things go quiet, not three scans of a folder while it is still being written to
  - Path mapping for the usual case where the two do not agree what a folder is called, which is almost always when either runs in a container. Test connection lists the folders your server itself reported, so you pick from a list rather than typing a path that fails silently
- Analyse your back catalogue overnight, on a schedule
  - Set hours when it may work through everything not yet analysed - midnight until 5am by default - and it gets on with it while you are not using the machine. Under Settings, Scheduled backfill
  - Mainly for local analysis, where one video can take tens of minutes and is better done at 3am. Scheduling Claude runs works too, where the point is deciding when you spend - so a nightly spending cap appears for those and not for local ones
  - The window decides when analyses may start, not when they are cut off: one already running at 5am finishes rather than being abandoned. It queues one video at a time, which is what keeps that overrun to a single video
  - Anything you start yourself jumps ahead of it, and a download finishing at 6am is not held up behind it
- It remembers what you have watched
  - Watched and part-watched markers on every row and card, with a progress bar along the thumbnail so you can see what you started and never finished
  - Filter by not started, part-watched or watched - the quickest way to find what is left in a back catalogue
  - Where you got to now survives closing the tab, and is kept by the app itself whether or not you run a media server
  - Plex and Jellyfin can keep it in step both ways. Watched is sticky, so a server that has never heard of a file cannot un-watch something you saw. Jellyfin needs only its API key - you pick the account from a list, no password
### Enhancements
- Transcription and local AI analysis now share one queue, under Settings then Local models
  - They cannot usefully run at the same time - each already uses most of your processor - so they take turns in a single queue you can see, reorder and cancel things in, none of which was possible while they were kept apart
  - Maximum simultaneous subtitle jobs has moved there and is now Maximum simultaneous local model jobs, carrying your existing value. It was never really about subtitles: an analysis wanting the machine would override it anyway
- The metadata backfill can now tell which files actually need it
  - Each file remembers what was written into it, so the list marks items Up to date, Out of date or Never written instead of offering everything with no way to tell them apart
  - Files downloaded before this existed read as Never written rather than up to date - nothing recorded what went into them, and claiming they are current would quietly skip the ones most likely to need it
- Updating a video's tags no longer rewrites the whole file, so a tag change is near-instant rather than a full rewrite of a multi-gigabyte download
- The content list shows at a glance what has been analysed and what has a written article, without opening each video
- The content panel is organised into tabs
  - Files, Analysis, Article, and Activity while something is running. Opening a video you have not downloaded shows the formats you can fetch, instead of four headings each saying there is nothing there
...
- A log you can actually read
  - The app writes its log to a file, with a Logs page under System to read it back, filter by level, search it and follow it live - so working out what happened during a download no longer means having the container console open at the time
...
- Media server connections are now three separate switches
  - Connecting to a server, telling it to rescan when files change, and keeping watched state in step are separate choices. A machine that never serves media has no use for rescans but every use for reading watched state, and previously the only way to get the second was to accept the first
  - Path mapping has moved up to sit with the server's address and key, since it is part of setting the connection up rather than of either job - and both break the same way when it is wrong
- Swipe between pages on a touch screen - the content panel's tabs, and the pages of Tools, Analysis and Settings
...
- Your browser no longer offers to save API keys as passwords. Keys and tokens are still hidden on screen, but are no longer the kind of field a password manager tries to remember - the app's own login is untouched
### Bug Fixes
...
- The check for new content now starts when the app does. It used to wait for everything else startup does - filling in YouTube details, scanning your download folder - which on a large library is hours, during which nothing new was noticed
- Transcriptions can be stopped. Once one started the only option was to wait it out, the better part of an hour on a long video. Stopping one cleans up the part-finished subtitle file and the audio it extracted
- Subtitle generation no longer looks stuck at the start of every run. Pulling the audio out and loading the speech model now report themselves, instead of sitting on an empty bar that reads as hung
- The Activity page makes more sense while a backfill runs: finished items are newest first, each says what it did and to which video rather than every row reading identically, and the count in the top bar includes background jobs
- Thumbnails are sharp again wherever they are shown large. Every one was fetched at 300 pixels and stretched - the request for a bigger image was written for the old site's address format and silently discarded
- The dismiss button on a notification works. It could be swiped away but the X did nothing, because pressing it started a drag on the notification behind it
- Pop-up panels reach the edges of a phone screen. The rule meant to make dialogs full width had been silently overridden since it was written, so it had never once worked
- The status column lines up again - a video with two downloaded files was pushing its whole status block sideways
### Security
- Updated a bundled library with a reported flaw in how it writes a generated identifier into a buffer. Nothing here ever called it that way, so it was not exploitable in practice - it is updated so the app does not ship a known-vulnerable version at all
- Reading a file's embedded metadata now only works for files this app downloaded. It previously trusted the filename it was handed, so a signed-in user could have pointed it at any file on the machine and read its details back
