...
- Or run it on your own machine instead, with no API key and nothing to pay
  ...
  - Two models to choose from, with the trade-off spelled out where you pick one. The smaller is the default and runs on a 12GB graphics card; the larger quotes the video more accurately, so more findings end up with a timestamp you can click, but it needs around 22GB of memory to hold and realistically a 32GB machine
