...
Your downloads can now be read as well as watched. Point it at an Anthropic API key and it will write a detailed summary and a separate verdict for each video, pull the hard numbers out into a table you can actually compare - per-platform resolutions and frame rates for a platform comparison, the recommended settings for a PC review - and suggest tags. It costs a few pence a video and tells you roughly what a run will cost before you spend it.
...
A new Analysis section reads across everything you have analysed rather than one video at a time - every platform comparison side by side, filterable by the platforms you care about, and an index of what was covered per game.
...
- A new Analysis section, reading across everything you have analysed
  ...
  - Port, patch and single-platform reviews appear in that table too, not just full face-offs. They used to be treated as a separate kind of analysis and left out, on a distinction that turned out not to hold: a "PS5 Pro tech review" often covers as many platforms as a face-off does, and the two were told apart from the title before anything had been read. They are now one kind, and whether a row is a face-off is worked out from the platforms actually found in it - so a row covering one platform is labelled as such rather than looking like a comparison that came back mostly empty
...
- Timestamps in an analysis opened from the Analysis section now take you to the video
  - Reading a platform comparison and wanting to see the moment being described used to be a dead end there - the timestamps only worked in a video's own panel, where there is a player beside them. Opening one now plays the video from that point
...
- A local analysis now shows its working as it goes, rather than only saying which step it is on. It reports the words it has written so far while it writes them, which on a slower machine is the difference between a run you can see progressing and one that looks stuck for ten minutes. The steps are weighted by how long they actually take, so the figure does not sit still through the longest part of the run
- Analysing locally no longer gives up after five minutes. The request was being abandoned while the model was still working perfectly well - on a slower machine a single long video could take longer than that, and the failure reported nothing more useful than "fetch failed". A local analysis is now given as long as it needs
- When a local analysis does fail, it says why - a model that took too long, a server that is not running, one that ran out of memory mid-answer - rather than the same unhelpful message for all of them
- Analysing with a local model no longer claims to be using Claude. The status showed whichever Anthropic model was configured regardless of which engine was actually running
- A local run now says what it is doing rather than sitting on "Analysing" for several minutes. The first ever run reports the model download as it goes, starting the model is announced rather than silent, and the analysis itself reports which of its steps it is on
...
- The metadata backfill dialog no longer shows an empty warning
  - Choosing to re-run one produced a warning box containing nothing but an exclamation mark. It now says what re-running actually does
  - It also no longer warns that the run will take a while when it will not - writing tags is now quick, so the dialog says so, and keeps the slow-and-steady warning for runs that include chapters, which genuinely do rewrite each file
- Every Analysis page can now open the analysis behind a row
  - PC settings, Hardware and Costs listed what had been found but gave you no way to read the full analysis it came from - only the games and platform comparison pages did. Clicking a title now opens it on all of them
- The new marks appear as soon as they are true
  - Analysing a video now updates its row straight away. Previously the result was there but the list had no idea, so it took reloading the page to see anything had happened
