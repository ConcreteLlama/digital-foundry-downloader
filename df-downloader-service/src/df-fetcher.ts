...
export const makeDfDownloadParams = (dfContent: DfContentInfo, mediaInfo: MediaInfo) => {
  ...
  const headers = {
    ...makeAuthHeaders(),
    // Mimicking a real browser navigation request here, not just picking
    // arbitrary values - confirmed live 2026-08-15 that a bare
    // cookie+distinctive-tool-UA request against videos/download/<id> got
    // the session blacklisted (redirected to /login, autologin actively
    // expired server-side) while the exact same cookie worked fine for a
    // real browser hitting the same URL. Deliberately omitting sec-ch-ua/
    // Client Hints - a slightly-off version pairing with User-Agent can
    // read as more suspicious than omitting them entirely.
