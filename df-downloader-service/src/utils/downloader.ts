...
class DownloadProgress {
  ...
  generateProgressReport(): DownloadProgressInfo {
    const now = new Date();
...
export type ProgressListener = (progressReport: DownloadProgressInfo) => void;

type DownloadInstanceOptions = {
  headers: Headers;
...
const defaultDownloadOptions: DownloadInstanceOptions = {
  headers: {},
...
export class DownloadInstance {
  ...
  protected readonly opts: DownloadInstanceOptions;
  constructor(
    protected readonly url: string,
    protected readonly destination: string,
    opts: Partial<DownloadInstanceOptions>
  ) {
    this.opts = {
...
export class MultiConnectionDownloadInstance {
  ...
  lastProgressReport?: DownloadProgressInfo;
  protected currentProgress: DownloadProgress = new DownloadProgress();
  private state: DownloadState = DownloadState.NOT_STARTED;
  ...
  constructor(
    ...
    private readonly opts: Partial<DownloadInstanceOptions>
  ) {}
  ...
  async download(): Promise<DownloadResult> {
    ...
    for (let i = 0; i < this.maxConnections; i++) {
      ...
      this.downloadInstances.push(
        new DownloadInstance(this.url, this.destination, {
          ...this.opts,
          range: {
            start,
            end: end,
...
export type DownloadOptions = Partial<
  DownloadInstanceOptions & {
    maxConnections?: number;
  }
>;

export async function download(url: string, destination: string, downloadOptions: Partial<DownloadOptions>) {
  const maxConnections = downloadOptions.maxConnections;
