...
- Can work through that backlog on a schedule instead, during hours you pick — set a window (midnight until 5am, say) and it feeds one video at a time into analysis while you're not using the machine. Aimed squarely at local analysis, where a single video can take tens of minutes and is only worth doing when the machine is idle
  - The window governs when analyses *start*, not when they're cut off: one already running at 5am finishes rather than being abandoned part-way. Feeding one at a time is what keeps that overrun to a single video rather than however many were queued at once
  - Scheduled work sits at the lowest priority, so anything you start yourself goes ahead of it
...
- Tells Plex or Jellyfin when files change, so a finished download appears in your library straight away instead of waiting for their next scheduled scan. Metadata written into a file, new subtitles and files moved by the Reorganize tool all count, and changes are batched so a server isn't told three times about one download
  - Connecting to a server, telling it to rescan, and keeping watched state in step are three separate switches — a machine that never serves media has no use for rescans but every use for reading watched state
  - Usually needs a path mapping, since the two sides rarely see the same folder at the same path when either runs in a container. The settings page tests the connection and lists the folders the server actually has, because a path it has never heard of fails silently
- Keeps track of what you've watched — its own, rather than a view onto a media server, since most installs have neither Plex nor Jellyfin and "have I seen this" is worth answering regardless. Rows and cards carry watched and part-watched markers with a progress bar, the list filters by not started / part-watched / watched, and your place in a video now survives closing the tab
  - Plex and Jellyfin can keep it in step both ways, pulled on a timer you set or on demand. Watched is sticky, so a server that has never indexed a file can't un-watch something you definitely saw
  - Jellyfin needs only its API key, and you pick which account from a list — no password
